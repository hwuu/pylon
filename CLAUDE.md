...
1. 方案评估请主动思考需求边界，合理质疑当下方案的完善性，方案需包含：重要逻辑的实现思路、需求按技术实现的依赖关系拆解并排序，便于后续渐进式开发、输出修改或新增文件的路径、输出测试要点利于需求完成后的自动化测试。

2. 方案讨论或代码编写时，如果遇到了争议或不确定性请主动告知我，请牢记让我决策而不是默认采用一种方案实现，重点强调。

3. 开发项目必须严格按步骤执行，每次只专注当前讨论的步骤，要求：不允许跨步骤实现功能或"顺便"完成其他步骤任务、实现前必须先确认技术方案和实现细节、每个步骤完成后必须明确汇报，等待 Review 确认后才能进入下一步。

4. 进行代码提交时，请先给我梳理提交的内容，等待 Review 确认后才能进行提交。

5. 与第3、4点类似，任何代码修改请始终遵守最小改动原则，除非我主动要求优化或者重构。

6. 代码实现请先思考哪些业务可以参考或复用，尽可能参考现有业务的实现风格，如果你不明确可让我为你提供，避免重复造轮子。

7. 不要在源码中插入mock的硬编码数据。

8. 同步更新相关文档。

9. 使用TDD开发模式开发。

10. 小步快跑，对一步都进行测试，并保证不影响现有用例。

11. 使用中文回答

12. 记得每次测试完后，清理下测试文件。

13. 在 bug 修复时如果超过 2 次修复失败，请主动添加关键日志后再进行尝试修复，在我反馈修复后主动清除之前的日志信息。

14. 项目中的重试过2次以上环境配置问题或其他重复犯错的问题，请在项目的CLAUDE.md中做记录。常用的命令，请记录在项目的CLAUDE.md中。

15. 及时在 tests/unit 中添加单元测试
...
进行了比较重要的修改之后，更新 docs/design.md

design.md 编写规范：
1. 标题有编号
2. 文档最前面有个目录
3. 按照类似 “背景、架构设计、数据结构设计、算法设计、测试方案、开发计划、扩展方向、附录” 这样来组织结构

更新 design.md 的时候，要更新标题编号和文档最前面的目录

文档中如果要画流程框图，那么框图中的文字用英文，框线要对齐；其余内容保持中文
